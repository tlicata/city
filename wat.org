...
once. That one process is started in oars:run_services(), but it is not
linked/monitored, so if it errors, it will die and not be restarted. I think
this should be turned into a supervisor & worker configuration with automatic
restarts, but I think this is good enough for now...
